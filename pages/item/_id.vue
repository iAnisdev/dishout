  ...
  <div class="page">
    ...
    <div v-for="group in item.radio_option_groups" :key="group.id">
      <Card style="margin-top: 2vh" :bordered="false">
        ...
        <p slot="extra" v-if="group.min_selectable && !selected">
          <Tag color="default">Required</Tag>
        </p>
        <p slot="extra" v-if="group.min_selectable && selected">
          <Icon type="ios-checkmark-circle" color="#19be6b" size="24"/>
        </p>
        <br>
  ...
  methods: {
    ...
    addtoCart(item) {
      ...
      if (item.radio_option_groups.length > 0 && that.modifier == "") {
        that.$Spin.hide();
        this.$Message.error(
          `Please ${item.radio_option_groups[0].description} first`
        );
      } else {
        if (that.orderMenu == null || that.orderMenu == menu_id) {
          ...
          this.$Modal.confirm({
            ...
            content:
              "<p>You can only order from on menu at a time , Clear your cart if you would like to order this item</p>",
            okText: "Clear Cart",
            cancelText: "Cancel",
            loading: true,
            onOk: () => {
