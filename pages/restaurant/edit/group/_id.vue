<template>
  <section class="page">
    ...
    <div class="content">
      <div class="menu menu_a">
        <div class="row">
          <div>
            ...
            <h2>
              ...
              <Tag color="error" type="border" v-if="!group.active">{{group.active | status}}</Tag>
            </h2>
          </div>
          <div>
        ...
        <Card dis-hover :bordered="false" width="100%" v-if="!showItems">
          <Row class="row--justify">No items Yet</Row>
        </Card>
    ...
    <Drawer title="Profile Info" placement="left" :closable="false" v-model="drawer">
      <div class="profile">
        ...
        <Menu active-name="1-2" class="menu-side" :open-names="['1']">
          ...
          <Submenu name="1">
            <template slot="title">
              <Icon type="ios-filing"/>Menu
            </template>
            <MenuItem name="1-1" @click.native="editModal = true , drawer = false">
              <Icon type="md-create"/>Edit Menu
            </MenuItem>
          </Submenu>
          <Submenu name="2">
            <template slot="title">
              <Icon type="ios-filing"/>Item
            </template>
    ...
    <Modal v-model="editModal" title="Edit Menu" @on-ok="updateMenuGroup()" @on-cancel="cancel()">
...
export default {
  ...
  methods: {
    ...
    addNewItem() {
      ...
      } else if (that.item.img_url == "") {
        that.$Message.warning(`Item Image Required`);
      } else {
        let data = {
          name: that.item.name,
